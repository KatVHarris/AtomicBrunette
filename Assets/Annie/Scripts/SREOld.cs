...
/// </summary>
public class SREOld : MonoBehaviour, SonicInterfaces.ConfigurationCallback, SonicInterfaces.ReadingTrackerCallback {
	...
	public void LoadNextPhrase(int phraseNum){
		Text[] words;
		words = GameObject.Find("/Dialog/Canvas/"+phraseNum).GetComponentsInChildren<Text>();
		lineTextList.Clear ();
		lineTextList.InsertRange (0, words);
		foreach (Text word in lineTextList) {
			 
			Debug.Log ("word in load: " + word.text);
		}
		RefreshDict ();
